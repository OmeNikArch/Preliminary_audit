...
<section>
  ...
  <table class="bench compact">
    <tr><th>Сценарий</th><th>Медиа / мес</th><th>Сопровождение 1-й / со 2-го мес</th><th>Итого / мес</th><th>За тест</th><th>Показы</th><th>Клики</th><th>CPC</th><th>CPC с сопровождением</th></tr>{trs}
  </table>
  </div>
  ...
  <div class="mark">Ц</div>
    ...
    for spend, label in TIERS:
        ...
        trs += f"""
      <tr><td{best}>{money(spend)}<br><span style="color:var(--mute);font-family:var(--body);font-weight:400;font-size:12px">{label}</span></td><td>{s1/spend*100:.0f}%</td><td>{s2/spend*100:.0f}%</td><td>{money(s1)}</td><td><b style="color:var(--yellow)">{money(s2)}</b></td><td>{money(s1 - s2)}</td><td>{money((s1 - s2) * 11)}</td></tr>"""
    s1m, s2m = srk(MAIN)
    s.append(f"""
<section>
  <div class="num">{nxt()}</div>
  ...
  <p class="sub">Сопровождение считается как доля от рекламного расхода в кабинете. Базовая ставка — 20%. С ростом оборота
  ставка со скидкой снижается ступенями: на вашем тестовом бюджете — <b>10%</b>, на верхней ступени — <b>2%</b>.
  Скидка начисляется на следующий месяц, если в текущем выдержан минимальный расход ступени.</p>
  <div style="overflow-x:auto">
  <table class="bench compact">
    <tr><th>Расход в месяц</th><th>Ставка без скидки</th><th>Ставка со скидкой</th><th>Сопровождение без скидки</th><th>Со скидкой</th><th>Экономия / мес</th><th>Экономия за 11 мес*</th></tr>{trs}
  </table>
  </div>
  <div class="cards" style="margin-top:26px">
    <div class="card"><h3>Как включается скидка</h3><p>Первый месяц — по базовой ставке 20%. Со второго месяца — ставка ступени, если расход предыдущего месяца не ниже её порога. Оплата сопровождения — раз в месяц.</p></div>
    <div class="card"><h3>На вашем бюджете {money(MAIN)}</h3><p>Сопровождение {money(s1m)} в первый месяц и {money(s2m)} дальше — экономия {money(s1m - s2m)} каждый месяц при сохранении оборота. Переход на 1,1 млн ₽ снижает ставку до 8%.</p></div>
  ...
  <div class="foot">* За год при сохранении оборота: 11 месяцев со скидкой, первый — без. Расход указан без НДС, стоимость сопровождения — с НДС. Ставка до 200 000 ₽ — фиксированные 50 000 ₽ в месяц.</div>
  <div class="mark">Ц</div>
</section>""")
