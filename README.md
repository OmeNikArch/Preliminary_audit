...
`sletat-ru-mediaplan.html` + `sletat-ru-mediaplan.xlsx` — медиаплан теста для Слетать.ру (запрос директора по маркетингу): два согласуемых источника (Ozon одобрен, Urban на согласовании, WB недоступен), три сценария бюджета, показы/клики по сегментам из benchmarks.csv, цена заявки при разной конверсии. Генератор — `mediaplan_sletat.py`.
