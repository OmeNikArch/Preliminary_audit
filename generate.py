...
def bench_table(client_key, source_key, niche_rows):
    ...
    # Если по сегменту своих цифр ещё нет, в CPM/CTR/CPC подставляем средние
    # по действующим клиентам СРК в этом источнике (BENCH_AVG) — по ним уже можно
    # прикинуть экономику. Объёмы (показы, уникальные, клики) так подставить нельзя:
    # они зависят от размера сегмента и бюджета, там остаётся «после сбора».
    UNIT_METRICS = ("cpm", "ctr", "cpc")
    avg = BENCH_AVG.get(source_key, {})
    ...
    for r in niche_rows:
        ...
        for m in _METRIC_ORDER:
            ...
            if v:
                filled_any = True
                cells += f"<td>{v}</td>"
            elif m in UNIT_METRICS and avg.get(m):
                filled_all = False
                used_avg = True
                cells += f'<td>≈ {avg[m]}</td>'
            else:
    ...
    if filled_all:
        note = ("<b>Бенчмарки сняты из рекламных кабинетов площадки</b> по всем выбранным сегментам; "
                "перед стартом значения сверяются прогнозаторами кабинетов.")
    elif filled_any:
        note = ("<b>Часть бенчмарков уже снята из кабинетов площадки</b> — оставшиеся ячейки будут "
                "добавлены в финальную версию аудита.")
    elif used_avg:
        note = ("<b>Значения со знаком ≈ — средние по действующим клиентам Церебро в этом источнике</b>, "
                "а не по вашим сегментам: по ним можно прикинуть экономику теста уже сейчас. "
                "Показы, уникальные пользователи и клики зависят от размера сегмента и бюджета — "
                "их снимаем в кабинетах площадки и добавляем в финальную версию аудита.")
    else:
        note = ("<b>Бенчмарки по выбранным сегментам собираются в рекламных кабинетах площадки</b> — "
...
<section>
  ...
  <div class="cards" style="margin-top:26px">
    <div class="card"><h3>Реклама внутри площадок — переход на ваш сайт</h3>
      <p>Ozon, Яндекс Urban Ads и Wildberries продают показы своей аудитории внешним рекламодателям:
      баннер человек видит внутри маркетплейса, а клик ведёт его из маркетплейса на ваш сайт —
      не в карточку товара.</p></div>
...
<section>
  ...
  <h2>Средние показатели клиентов<br>на сопровождении в Click Out</h2>
  <div class="cards">
  ...
  Данные основаны на реальных кампаниях клиентов на сопровождении; показатели по вашей нише будут уточнены после сбора бенчмарков по выбранным сегментам.</div>
  <div class="mark">Ц</div>
</section>""")
...
<section>
  ...
  <div class="mark">Ц</div>
    ...
    # ---------- Шкала премирования Ozon Performance ----------
    s.append(f"""
<section>
  <div class="num">{num()}</div>
  <div class="kicker"><div class="bar"></div><span>Ozon Performance · Условия</span></div>
  <h2>Шкала премирования клиентов Церебро<br>в направлении Ozon Performance</h2>
  <div class="cards" style="margin-top:10px">
    <div class="card">
      <h3>Рессейлинг — кабинет ведёте сами: бонусные рубли с расхода</h3>
      <table class="bench">
        <tr><th>Расход в месяц, без НДС</th><th>Бонусные рубли клиенту</th></tr>
        <tr><td>до 360 000 ₽</td><td>0%</td></tr>
        <tr><td>360 001 — 1 099 999 ₽</td><td>10%</td></tr>
        <tr><td>1 100 000 — 1 999 999 ₽</td><td>12%</td></tr>
        <tr><td>2 000 000 — 6 999 999 ₽</td><td>14%</td></tr>
        <tr><td>7 000 000 — 9 999 999 ₽</td><td>16%</td></tr>
        <tr><td>от 10 000 000 ₽</td><td>18%</td></tr>
      </table>
    </div>
    <div class="card">
      <h3>Сопровождение — кабинет ведёт специалист Церебро: стоимость со скидкой</h3>
      <table class="bench">
        <tr><th>Расход в месяц, без НДС</th><th>Сопровождение без скидки</th><th>Со скидкой</th></tr>
        <tr><td>до 200 000 ₽</td><td>50 000 ₽</td><td>50 000 ₽</td></tr>
        <tr><td>200 001 — 360 000 ₽</td><td>20%</td><td>20%</td></tr>
        <tr><td>360 001 — 1 099 999 ₽</td><td>20%</td><td>10%</td></tr>
        <tr><td>1 100 000 — 1 999 999 ₽</td><td>20%</td><td>8%</td></tr>
        <tr><td>2 000 000 — 6 999 999 ₽</td><td>20%</td><td>6%</td></tr>
        <tr><td>7 000 000 — 9 999 999 ₽</td><td>20%</td><td>4%</td></tr>
        <tr><td>от 10 000 000 ₽</td><td>20%</td><td>2%</td></tr>
      </table>
    </div>
  </div>
  <div class="foot">Бонусные рубли начисляются раз в квартал — в течение 30 дней по окончании квартала — при суммарном
  квартальном расходе в рекламном источнике от 360 001 ₽. Минимальная оплата сопровождения — раз в месяц с даты оплаты
  услуг, независимо от расхода в кабинете; скидка на сопровождение начисляется ежемесячно на следующий месяц при
  соблюдении минимального расхода в текущем. При работе с сопровождением бонусные рубли не начисляются — действует только
  скидка на сопровождение; вариант «сопровождение + бонусные рубли» невозможен. Стоимость сопровождения указана с НДС.</div>
  <div class="mark">Ц</div>
</section>""")
